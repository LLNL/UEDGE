...
c-----------------------------------------------------------------------
      subroutine myitoa(n, str, strlen)
      implicit none
      integer n, strlen
...
c ... Output Jacobian matrix in MatrixMarket format.
...
c ... Local variables:
      ...
      character(len=10) Y
      integer jacii(nnzmx), nnz
c
      mmcount = mmcount + 1
      call myitoa(mmcount, Y, len(Y))
c ... Open a file, and output data.
      call freeus (us)
      filename='UedgeJacobian'//trim(adjustl(Y))//'.mtx'
      open(unit=us, file=filename, status='unknown')
c
      nnz = jaci(neq+1) - 1
      do i = 1, nnz
      ...
      do i = 1, neq
         do j = jaci(i), jaci(i+1)-1
            jacii(j) = i
         end do
      end do

      call mmwrite (us, 'coordinate', 'real', 'general', neq, neq, nnz, jacii, jacj, jac, jac, jac)

c ... Close file, and report file name.
      close(us)
      write (STDOUT,*) ' Jacobian matrix in mtx file saved in ', filename
