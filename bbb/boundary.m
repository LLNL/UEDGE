...
ccc
     ...
     .                  + outflux_atom(
     .                      fniy_recy,
     .                      osmw*harmave(ng(ix,ny,igsp), ng(ix,ny+1,igsp)),
     .                      recycwot(ix,igsp),
     .                      albedoo(ix,igsp)
     .                  )
     .                  + fng_chem 
     ...
     .              ) / (osmw*n0g(igsp))
                ELSE
                    IF (igsp .eq. 2) THEN
                        if (isupgon(1) .eq. 1) then
                            fniy_recy = 
     .                          0.5*( fniy(ix,ny,1) + fniy(ix,ny,2) )
                        else
                            fniy_recy = 
     .                          0.5*( fniy(ix,ny,1) + fngy(ix,ny,1) )
                        endif
                        if(isrefluxclip==1)
     .                      fniy_recy=max(fniy_recy,0.)
                    END IF
...
c       Do hydrogenic gas equations --
        IF (ishymol .ne. 1) THEN
          IF (isngonxy(ixt,iy,1) .eq. 1) then
            iv = idxg(ixt,iy,1)
            osmw = onesided_maxwellian(
     .                  tg(ixt1,iy,1), ng(ixt1,iy,1), mg(1),
     .                  isoldalbarea*sx(ixt,iy) + (1-isoldalbarea)*sxnp(ixt,iy),
...
c       Atom-only, implicit molecules
                iv = idxg(ixt,iy,1)
                osmw = onesided_maxwellian(
     .              tg(ixt1,iy,1), 1.0, mg(1),
     .              isoldalbarea*sx(ixt1,iy) + (1-isoldalbarea)*sxnp(ixt1,iy),
     .              tgmin*ev
     .         )
            
                yldot(iv) = nurlxg *  ( fngx(ixt1,iy,1)
     .                  + fngxrb_use(iy,1,jx)
     .                  - fngxsrb(iy,1,jx) 
     .                  + outflux_atom(
