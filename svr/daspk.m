...
C
      SUBROUTINE SDASPK (RES, NEQ, T, Y, YPRIME, TOUT, INFO, RTOL, ATOL,
     *   IDID, RWORK, LRW, IWORK, LIW, RPAR, IPAR, JAC, PSOL, scalpk)
C
C***BEGIN PROLOGUE  SDASPK
...
C***REVISION DATE  950125   (Added SINVWT routine)
...
C***REVISION DATE  950824   (Converted from double to single precision.)
C***REVISION DATE  950828   (Various minor corrections.)
C***REVISION DATE  951006   (Corrected WT scaling in SFNRMK.)
C***REVISION DATE  960401   (Corrected RL undefined in SLINSK and SLINSD.)
C***REVISION DATE  960418   (Added optional input EPNL when INFO(13)=1.)
C***CATEGORY NO.  I1A2
...
C      real T, Y(*), YPRIME(*), TOUT, RTOL(*), ATOL(*),
C         RWORK(LRW), RPAR(*)
C      EXTERNAL  RES, JAC, PSOL
C
C      CALL SDASPK (RES, NEQ, T, Y, YPRIME, TOUT, INFO, RTOL, ATOL,
...
C
c  scalpk:in        This parameter is introduced (by Tom Rognlien and
c                   Jose Milovich) to compensate for large rtol in the
c                   calculation of the preconditioner in the routine SATV
c                   
...
C *Overview
C
C  The SDASPK solver uses the backward differentiation formulas of
...
C  known, in many cases you can have SDASPK solve for them -- see INFO(11).
C  (This and other options are described in more detail below.)
C
C  Normally, SDASPK solves the system from T to TOUT.  It is easy to
...
C
C  On each step taken by SDASPK, a sequence of nonlinear algebraic  
...
C  (SDASPK does not allow right preconditioning.)
...
C *Description
C
C------INPUT - WHAT TO DO ON THE FIRST CALL TO SDASPK-------------------
C
C
...
C         statement in your program that calls SDASPK.
...
C         Note: In this respect, SDASPK is not downward-compatible
C         with SDASSL, which does not have the RES argument CJ.
C
...
C         Set IRES = -1 if an input value is illegal, and SDASPK
C         will try to solve the problem without getting IRES = -1.
C         If IRES = -2, SDASPK will return control to the calling
C         program with IDID = -11.
C
C         RPAR and IPAR are real and integer parameter arrays which
C         you can use for communication between your calling program
C         and subroutine RES. They are not altered by SDASPK. If you
...
C              SDASPK will perform much better if the matrix of
C              partial derivatives, dG/dY + CJ*dG/dYPRIME (here CJ is
C              a scalar determined by SDASPK), is banded and the code
...
C
C       INFO(11) - SDASPK normally requires the initial T, Y, and
...
C              SDASPK may be able to compute it.
C
C              Denoting the differential variables in Y by Y_d
C              and the algebraic variables by Y_a, SDASPK can solve
...
C              SDASPK by default is downward-compatible with SDASSL,
...
C          ****   Do you want SDASPK to use standard direct methods
...
C
C       INFO(13) - used for optional inputs to both direct and Krylov
C              method cases (INFO(12) = 0 or 1).  When INFO(12) = 0
C              or 1, EPNL is used as a swing factor in the Newton
C              iteration convergence test.  When INFO(12) = 1,
C              SDASPK uses scalars MAXL, KMP, NRMAX, and EPLI for the
...
C              For INFO(12) = 0 or 1,
C              EPNL = swing factor in the Newton iteration convergence
...
C                 less than EPNL*EPCON, where EPCON = .33 is the
C                 analogous test constant used in the time steps.
C                 The default is EPNL = 1.
C              For INFO(12) = 1,
...
C          ****   Are EPNL, MAXL, KMP, and EPLI to be given their
C                 default values ...
C                  yes - set INFO(13) = 0
C                   no - set INFO(13) = 1,
C                        and set all of the following:
C                        RWORK(16) = EPNL (.GT. 0.0)  ****
...
C               The usage of SDASPK is simpler if PSOL can carry out
...
C
C       INFO(17) - used when INFO(11) > 0 (SDASPK is to do an 
C              initial condition calculation).
C              SDASPK uses several heuristic control quantities in the
...
C              base = 50 + (MAXORD+5)*NEQ + (MAXL+3+MIN(1,MAXL-KMP))*NEQ +
...
C
C
C  RPAR, IPAR -- These are arrays of single precision and integer type,
C             respectively, which are available for you to use
C             for communication between your program that calls
C             SDASPK and the RES subroutine (and the JAC and PSOL
C             subroutines).  They are not altered by SDASPK.
...
C           RTOL*abs(Y(i)) + ATOL (unless you supplied routine SDAWTS
...
C           forms).  They are not altered by SDASPK.
...
C         your program that calls SDASPK.
C
...
C
C  SDASPK uses a weighted root-mean-square norm to measure the 
C  size of various error vectors.  The weights used in this norm
C  are set in the following subroutine:
C
C    SUBROUTINE SDAWTS (NEQ, IWT, RTOL, ATOL, Y, EWT, RPAR, IPAR)
C    DIMENSION RTOL(*), ATOL(*), Y(*), EWT(*), RPAR(*), IPAR(*)
C
C  A SDAWTS routine has been included with SDASPK which sets the
...
C  your own error weights by writing a subroutine SDAWTS to be 
...
C  norm used in SDASPK (weighted root-mean-square) is not subject
C  to replacement by the user.  In this respect, SDASPK is not
C  downward-compatible with the original SDASSL solver (in which
C  the norm routine was optionally user-replaceable).
C
C
C------OUTPUT - AFTER ANY RETURN FROM SDASPK----------------------------
C
...
C
C           IDID = -6 -- SDASPK had repeated error test failures on the
...
C
C           IDID =-12 -- SDASPK failed to compute the initial Y, YPRIME.
C
...
C                  attempted step in SDASPK.  A singularity in the
...
C                  attempted step in SDASPK.  An inaccurate or ill-
...
C                  may be redundant.  SDASPK cannot solve the problem
C                  as stated.  It is possible that the redundant
C                  equations could be removed, and then SDASPK could
C                  solve the problem.  It is also possible that a
C                  solution to your problem either does not exist
C                  or is not unique.
C
C     IDID = -9, SDASPK had multiple convergence test failures, preceded
...
C
C     IDID = -10, SDASPK had multiple convergence test failures
...
C
C     IDID = -12, SDASPK failed to compute the initial Y and YPRIME
...
C***ROUTINES CALLED
C
C   The following are all the subordinate routines used by SDASPK.
C
C   SDASIC computes consistent initial conditions.
C   SYYPNW updates Y and YPRIME in linesearch for initial condition
C          calculation.
C   SDSTP  carries out one step of the integration.
C   SCNSTR/SCNST0 check the current solution for constraint violations.
C   SDAWTS sets error weight quantities.
C   SINVWT tests and inverts the error weights.
C   SDATRP performs interpolation to get an output solution.
C   SDWNRM computes the weighted root-mean-square norm of a vector.
C   R1MACH8 provides the unit roundoff of the computer.
C   XERRWV/XSETF/XSETUN/IXSAV is a package to handle error messages. 
C   SDASID nonlinear equation driver to initialize Y and YPRIME using
C          direct linear system solver methods.  Interfaces to Newton
C          solver (direct case).
C   SNSID  solves the nonlinear system for unknown initial values by
C          modified Newton iteration and direct linear system methods.
C   SLINSD carries out linesearch algorithm for initial condition
C          calculation (direct case).
C   SFNRMD calculates weighted norm of preconditioned residual in
C          initial condition calculation (direct case).
C   SNEDD  nonlinear equation driver for direct linear system solver
C          methods.  Interfaces to Newton solver (direct case).
C   SMATD  assembles the iteration matrix (direct case).
C   SNSD   solves the associated nonlinear system by modified
C          Newton iteration and direct linear system methods.
C   SSLVD  interfaces to linear system solver (direct case).
C   SDASIK nonlinear equation driver to initialize Y and YPRIME using
C          Krylov iterative linear system methods.  Interfaces to
C          Newton solver (Krylov case).
C   SNSIK  solves the nonlinear system for unknown initial values by
C          Newton iteration and Krylov iterative linear system methods.
C   SLINSK carries out linesearch algorithm for initial condition
C          calculation (Krylov case).
C   SFNRMK calculates weighted norm of preconditioned residual in
C          initial condition calculation (Krylov case).
C   SNEDK  nonlinear equation driver for iterative linear system solver
C          methods.  Interfaces to Newton solver (Krylov case).
C   SNSK   solves the associated nonlinear system by Inexact Newton
C          iteration and (linear) Krylov iteration.
C   SSLVK  interfaces to linear system solver (Krylov case).
C   SSPIGM solves a linear system by SPIGMR algorithm.
C   SATV   computes matrix-vector product in Krylov algorithm.
C   SORTH  performs orthogonalization of Krylov basis vectors.
C   SHEQR  performs QR factorization of Hessenberg matrix.
C   SHELS  finds least-squares solution of Hessenberg linear system.
C   SGEFA, SGESL, SGBFA, SGBSL are LINPACK routines for solving 
C          linear systems (dense or band direct methods).
C   SAXPY, SCOPY, SDOT, SNRM2, SSCAL are Basic Linear Algebra (BLAS)
C          routines.
C
C The routines called directly by SDASPK are:
C   SCNST0, SDAWTS, SINVWT, R1MACH8, SDWNRM, SDASIC, SDATRP, SDSTP,
C   XERRWV
C
C***END PROLOGUE SDASPK
C
C
      IMPLICIT real(A-H,O-Z)
      INTEGER LRW,LIW,LML,LMU,LMTYPE,LIWM,LMXORD,LJCALC,LPHASE, 
     *   LK,LKOLD,LNS,LNSTL,LNST,LNRE,LNJE,LETF,LNCFN,
     *   LNCFL,LNIW,LNRW,LNNI,LNLI,LNPS,LNPD,LMITER,LMAXL,
     *   LKMP,LNRMAX,LLNWP,LLNIWP,LLOCWP,LLCIWP,LKPRIN,
     *   LMXNIT,LMXNJ,LMXNH,LLSOFF,LICNS,INFO

      INTEGER LTSTOP,LHMAX,LH,LTN,LCJ,LCJOLD,LHOLD,LS,LROUND,
     *   LEPLI,LSQRN,LRSQRN,LEPCON,LSTOL,LEPIN,LEPNL,
     *   LALPHA,LBETA,LGAMMA,LPSI,LSIGMA,LDELTA

      INTEGER I,ITEMP,NEQ,MXORD,IWORK,ICNFLG,NONNEG,LID,LSOFF,
     *  LENIC,LENID,NCPHI,LENPD,LENRW,MBAND,MSAVE,LENIW,LENWP,
     *  LENIWP,MAXL,ICI,IRET,IDI,IDID,NLI0,NNI0,NCFN0,NCFL0,NWARN,
     *  NZFLG,LE,LWT,LVT,LPHI,LWM,IER,NWT,LYIC,LYPIC,LPWK,NSTD
      INTEGER NNID,LSAVR,IPAR
      ...
      DIMENSION RPAR(*),IPAR(*)
      CHARACTER MSG*80
      EXTERNAL  RES, JAC, PSOL, SDASID, SDASIK, SNEDD, SNEDK
      common /comsatv/ scaleps
C
...
C
     ...
     *   LEPCON=13, LSTOL=14, LEPIN=15, LEPNL=16,
     *   LALPHA=21, LBETA=27, LGAMMA=33, LPSI=39, LSIGMA=45, LDELTA=51)
C
      SAVE LID, LENID
C
C
C***FIRST EXECUTABLE STATEMENT  SDASPK
C
C
      scaleps = scalpk
...
C
     ...
     1      GO TO 721
         ...
         IF(RWORK(LEPLI).LE.0.0E0 .OR. RWORK(LEPLI).GE.1.0E0)GO TO 723
         ENDIF
C
 25   CONTINUE
C
C     Set the correct value of EPNL, stored in RWORK(LEPNL).
C
      IF (INFO(13) .EQ. 0) THEN
         RWORK(LEPNL) = 1.0E0
      ELSE
         IF(RWORK(LEPNL) .LE. 0.0E0) GO TO 728
      ENDIF
C
...
C
      ...
      ELSE
        ...
        IF (RWORK(LEPIN) .LE. 0.0E0) GO TO 725
        ENDIF
C
 30   CONTINUE
C
...
C
      ELSE IF (INFO(12) .EQ. 1)  THEN
         ...
         LENWP = IWORK(LLNWP)
         LENIWP = IWORK(LLNIWP)
         LENPD = (MAXL+3+MIN(1,MAXL-IWORK(LKMP)))*NEQ
...
C
      IF (LENIC .GT. 0) THEN
        CALL SCNST0(NEQ,Y,IWORK(LICNS),IRET)
...
C     condition from SDSTP, and appropriate action was not taken.
C     This is a fatal error.
C
      MSG = 'DASPK--  THE LAST STEP TERMINATED WITH A NEGATIVE'
      CALL XERRWV(MSG,49,201,0,0,0,0,0,0.0E0,0.0E0)
      MSG = 'DASPK--  VALUE (=I1) OF IDID AND NO APPROPRIATE'
      CALL XERRWV(MSG,47,202,0,1,IDID,0,0,0.0E0,0.0E0)
      MSG = 'DASPK--  ACTION WAS TAKEN. RUN TERMINATED'
      CALL XERRWV(MSG,41,203,1,0,0,0,0,0.0E0,0.0E0)
      RETURN
110   CONTINUE
C
C-----------------------------------------------------------------------
...
C     
      IF (INFO(11) .NE. 0) THEN
        IF( INFO(17) .EQ. 0) THEN
          RWORK(LSTOL) = UROUND**.6667E0
        ELSE
          IF (RWORK(LSTOL) .LE. 0.0E0) GO TO 725
          ENDIF
        ENDIF
C
...
C
      IF (INFO(8) .EQ. 0) GO TO 310
         H0 = RWORK(LH)
         IF ((TOUT - T)*H0 .LT. 0.0E0) GO TO 711
         IF (H0 .EQ. 0.0E0) GO TO 712
         GO TO 320
310    CONTINUE
C
C     Compute initial stepsize, to be used by either
C     SDSTP or SDASIC, depending on INFO(11).
C
      H0 = 0.001E0*TDIST
      YPNORM = SDWNRM(NEQ,YPRIME,RWORK(LVT),RPAR,IPAR)
      IF (YPNORM .GT. 0.5E0/H0) H0 = 0.5E0/YPNORM
      H0 = SIGN(H0,TOUT-T)
C
...
330   IF (INFO(4) .EQ. 0) GO TO 340
         TSTOP = RWORK(LTSTOP)
         IF ((TSTOP - T)*H0 .LT. 0.0E0) GO TO 715
         IF ((T + H0 - TSTOP)*H0 .GT. 0.0E0) H0 = TSTOP - T
         IF ((TSTOP - TOUT)*H0 .LT. 0.0E0) GO TO 709
C
...
350   IF (INFO(12) .EQ. 0) THEN
         ...
         LYPIC = LYIC + NEQ
         LPWK = LYPIC
         CALL SDASIC(TN,Y,YPRIME,NEQ,INFO(11),IWORK(LID),
     ...
     *     EPCONI,RWORK(LSTOL),INFO(15),ICNFLG,IWORK(LICNS),SDASID)
      ELSE IF (INFO(12) .EQ. 1) THEN
         LYIC = LWM
         LYPIC = LYIC + NEQ
         LPWK = LYPIC + NEQ
     ...
     *     RWORK(LYIC),RWORK(LYPIC),RWORK(LPWK),RWORK(LWM),IWORK(LIWM),
     *     HMIN,RWORK(LROUND),RWORK(LEPLI),RWORK(LSQRN),RWORK(LRSQRN),
     *     EPCONI,RWORK(LSTOL),INFO(15),ICNFLG,IWORK(LICNS),SDASIK)
      ENDIF
C
      IF (IDID .LT. 0) GO TO 600
C
C     SDASIC was successful.  If this was the first call to SDASIC,
...
C
 ...
 357      RWORK(LVT+I-1) = MAX(IWORK(LID+I-1),0)*RWORK(LWT+I-1)
        ENDIF
C
C     Reset the initial stepsize to be used by SDSTP.
...
C
C-----------------------------------------------------------------------
C     The next block contains the call to the one-step integrator SDSTP.
...
505   IF (INFO(12) .EQ. 0) GO TO 510
      ...
      IF (LAVL) THEN
        ...
        CALL XERRWV (MSG, 56, 501, 0, 0, 0, 0, 0, 0.0E0, 0.0E0)
        MSG = '      at T = R1. Average no. of linear iterations = R2  '
        CALL XERRWV (MSG, 56, 501, 0, 0, 0, 0, 2, TN, AVLIN)
        ENDIF
      IF (LCFN) THEN
        MSG = 'DASPK-- Warning. Poor iterative algorithm performance   '
        CALL XERRWV (MSG, 56, 502, 0, 0, 0, 0, 0, 0.0E0, 0.0E0)
        MSG = '      at T = R1. Nonlinear convergence failure rate = R2'
        CALL XERRWV (MSG, 56, 502, 0, 0, 0, 0, 2, TN, RCFN)
        ENDIF
      IF (LCFL) THEN
        MSG = 'DASPK-- Warning. Poor iterative algorithm performance   '
        CALL XERRWV (MSG, 56, 503, 0, 0, 0, 0, 0, 0.0E0, 0.0E0)
        MSG = '      at T = R1. Linear convergence failure rate = R2   '
        CALL XERRWV (MSG, 56, 503, 0, 0, 0, 0, 2, TN, RCFL)
        ENDIF
C
C     Update WT and VT, if this is not the first call.
C
510   CALL SDAWTS(NEQ,INFO(2),RTOL,ATOL,RWORK(LPHI),RWORK(LWT),
     *            RPAR,IPAR)
      CALL SINVWT(NEQ,RWORK(LWT),IER)
      IF (IER .NE. 0) THEN
        IDID = -3
...
C
      R = SDWNRM(NEQ,RWORK(LPHI),RWORK(LWT),RPAR,IPAR)*100.0E0*UROUND
...
C
      EPCON = RWORK(LEPNL)*RWORK(LEPCON)
      IF (INFO(12) .EQ. 0) THEN
         CALL SDSTP(TN,Y,YPRIME,NEQ,
     ...
     *      EPCON, IWORK(LPHASE),IWORK(LJCALC),INFO(15),
     *      IWORK(LK), IWORK(LKOLD),IWORK(LNS),NONNEG,INFO(12),
     *      SNEDD,RWORK(1),IWORK(1))
c ... Last two arguments are added to accommodate the run-time diagnostics
      ELSE IF (INFO(12) .EQ. 1) THEN
         CALL SDSTP(TN,Y,YPRIME,NEQ,
     *      RES,JAC,PSOL,H,RWORK(LWT),RWORK(LVT),INFO(1),IDID,RPAR,IPAR,
     *      RWORK(LPHI),RWORK(LSAVR),RWORK(LDELTA),RWORK(LE),
...
C
C-----------------------------------------------------------------------
C     This block handles the case of a successful return from SDSTP
...
C
C-----------------------------------------------------------------------
C     All successful returns from SDASPK are made from this block.
C-----------------------------------------------------------------------
C
590   CONTINUE
      RWORK(LTN)=TN
...
640   MSG = 'DASPK--  AT T (=R1) AND STEPSIZE H (=R2) THE'
      ...
      CALL XERRWV(MSG,57,641,0,0,0,0,0,0.0E0,0.0E0)
      GO TO 700
C
...
660   MSG = 'DASPK--  AT T (=R1) AND STEPSIZE H (=R2) THE'
      ...
      CALL XERRWV(MSG,38,661,0,0,0,0,0,0.0E0,0.0E0)
      GO TO 700
C
...
670   MSG = 'DASPK--  AT T (=R1) AND STEPSIZE H (=R2) THE'
      ...
      CALL XERRWV(MSG,45,671,0,0,0,0,0,0.0E0,0.0E0)
      MSG = 'DASPK--  ALSO, THE ERROR TEST FAILED REPEATEDLY.'
      CALL XERRWV(MSG,49,672,0,0,0,0,0,0.0E0,0.0E0)
      GO TO 700
C
...
675   MSG = 'DASPK--  AT T (=R1) AND STEPSIZE H (=R2) THE'
      ...
      CALL XERRWV(MSG,51,676,0,0,0,0,0,0.0E0,0.0E0)
      MSG = 'DASPK--  BECAUSE IRES WAS EQUAL TO MINUS ONE'
      CALL XERRWV(MSG,44,677,0,0,0,0,0,0.0E0,0.0E0)
      GO TO 700
C
...
C     as detected before calling SDSTP.
...
717   MSG = 'DASPK--  ML (=I1) ILLEGAL. EITHER .LT. 0 OR .GT. NEQ'
      CALL XERRWV(MSG,52,17,0,1,IWORK(LML),0,0,0.0E0,0.0E0)
      GO TO 750
718   MSG = 'DASPK--  MU (=I1) ILLEGAL. EITHER .LT. 0 OR .GT. NEQ'
      CALL XERRWV(MSG,52,18,0,1,IWORK(LMU),0,0,0.0E0,0.0E0)
      GO TO 750
...
720   MSG = 'DASPK--  MAXL (=I1) ILLEGAL. EITHER .LT. 1 OR .GT. NEQ'
      CALL XERRWV(MSG,54,20,0,1,IWORK(LMAXL),0,0,0.0E0,0.0E0)
      GO TO 750
721   MSG = 'DASPK--  KMP (=I1) ILLEGAL. EITHER .LT. 1 OR .GT. MAXL'
      CALL XERRWV(MSG,54,21,0,1,IWORK(LKMP),0,0,0.0E0,0.0E0)
      GO TO 750
722   MSG = 'DASPK--  NRMAX (=I1) ILLEGAL. .LT. 0'
      CALL XERRWV(MSG,36,22,0,1,IWORK(LNRMAX),0,0,0.0E0,0.0E0)
      GO TO 750
723   MSG = 'DASPK--  EPLI (=R1) ILLEGAL. EITHER .LE. 0.E0 OR .GE. 1.E0'
      CALL XERRWV(MSG,58,23,0,0,0,0,1,RWORK(LEPLI),0.0E0)
      GO TO 750
724   MSG = 'DASPK--  ILLEGAL IWORK VALUE FOR INFO(11) .NE. 0'
      CALL XERRWV(MSG,48,24,0,0,0,0,0,0.0E0,0.0E0)
      GO TO 750
725   MSG = 'DASPK--  ONE OF THE INPUTS FOR INFO(17) = 1 IS ILLEGAL'
      CALL XERRWV(MSG,54,25,0,0,0,0,0,0.0E0,0.0E0)
      GO TO 750
726   MSG = 'DASPK--  ILLEGAL IWORK VALUE FOR INFO(10) .NE. 0'
      CALL XERRWV(MSG,48,26,0,0,0,0,0,0.0E0,0.0E0)
      GO TO 750
727   MSG = 'DASPK--  Y(I) AND IWORK(40+I) (I=I1) INCONSISTENT'
      CALL XERRWV(MSG,49,27,0,1,IRET,0,0,0.0E0,0.0E0)
      GO TO 750
728   MSG = 'DASPK--  EPNL (=R1) ILLEGAL. IT MUST BE .GT. 0.E0'
      CALL XERRWV(MSG,49,23,0,0,0,0,1,RWORK(LEPNL),0.0E0)
      GO TO 750
...
760   MSG = 'DASPK--  REPEATED OCCURRENCES OF ILLEGAL INPUT'
      CALL XERRWV(MSG,46,701,0,0,0,0,0,0.0E0,0.0E0)
770   MSG = 'DASPK--  RUN TERMINATED. APPARENT INFINITE LOOP'
      CALL XERRWV(MSG,47,702,1,0,0,0,0,0.0E0,0.0E0)
      RETURN
C
C------END OF SUBROUTINE SDASPK-----------------------------------------
      END
C Work performed under the auspices of the U.S. Department of Energy
C by Lawrence Livermore National Laboratory under contract number 
C W-7405-Eng-48.
...
C All rights reserved.
C
      SUBROUTINE SDASIC(X,Y,YPRIME,NEQ,ICOPT,ID,RES,JAC,PSOL,H,WT,NIC,
     *   IDID,RPAR,IPAR,PHI,SAVR,DELTA,E,YIC,YPIC,PWK,WM,IWM,HMIN,
     *   UROUND,EPLI,SQRTN,RSQRTN,EPCONI,STPTOL,JFLG,ICNFLG,ICNSTR,
     *   NLSIC)
C
C***BEGIN PROLOGUE  SDASIC
C***REFER TO  SDASPK
...
C***DESCRIPTION
C
C     SDASIC is a driver routine to compute consistent initial values
...
C                  residual.  See RES description in SDASPK prologue.
C     JAC --       External user-supplied routine to update Jacobian
C                  or preconditioner information in the nonlinear solver
C                  (optional).  See JAC description in SDASPK prologue.
C     PSOL --      External user-supplied routine to solve
C                  a linear system using preconditioning. 
C                  See PSOL in SDASPK prologue.
C     H --         Scaling factor in iteration matrix.  SDASIC may 
C                  reduce H to achieve convergence.
C     WT --        Vector of weights for error criterion.
C     NIC --       Input number of initial condition calculation call 
C                  (= 1 or 2).
C     IDID --      Completion code.  See IDID in SDASPK prologue.
C     RPAR,IPAR -- Real and integer parameter arrays that
C                  are used for communication between the
C                  calling program and external user routines.
C                  They are not altered by SNSK
C     PHI --       Work space for SDASIC of length at least 2*NEQ.
C     SAVR --      Work vector for SDASIC of length NEQ.
C     DELTA --     Work vector for SDASIC of length NEQ.
C     E --         Work vector for SDASIC of length NEQ.
C     YIC,YPIC --  Work vectors for SDASIC, each of length NEQ.
C     PWK --       Work vector for SDASIC of length NEQ.
...
C
C     The other parameters are for use internally by SDASIC.
C
C-----------------------------------------------------------------------
C***ROUTINES CALLED
C   SCOPY, NLSIC
C
C***END PROLOGUE  SDASIC
C
C
      IMPLICIT real(A-H,O-Z)
      INTEGER IPAR,NEQ,LCFN,LMXNH,MXNH,IWM,IDID,NH,JSKIP,NIC,ICOPT,
     *  IERNLS,ICNFLG,JFLG,ID,ICNSTR
...
C
      ...
      JSKIP = 0
      IF (NIC .EQ. 2) JSKIP = 1
      CALL SCOPY (NEQ, Y, 1, PHI(1,1), 1)
      CALL SCOPY (NEQ, YPRIME, 1, PHI(1,2), 1)
C
      IF (ICOPT .EQ. 2) THEN
        CJ = 0.0E0 
      ELSE
...
C
      CALL SCOPY (NEQ, PHI(1,1), 1, Y, 1)
      CALL SCOPY (NEQ, PHI(1,2), 1, YPRIME, 1)
      GO TO 200
C
 350  IDID = -12
      RETURN
C
C------END OF SUBROUTINE SDASIC-----------------------------------------
      END
C Work performed under the auspices of the U.S. Department of Energy
C by Lawrence Livermore National Laboratory under contract number 
C W-7405-Eng-48.
...
C All rights reserved.
C
      SUBROUTINE SYYPNW (NEQ, Y, YPRIME, CJ, RL, P, ICOPT, ID, 
     *                   YNEW, YPNEW)
C
C***BEGIN PROLOGUE  SYYPNW
C***REFER TO  SLINSK
...
C***DESCRIPTION
C
C     SYYPNW calculates the new (Y,YPRIME) pair needed in the
C     linesearch algorithm based on the current lambda value.  It is
C     called by SLINSK and SLINSD.  Based on the ICOPT and ID values,
...
C
C***END PROLOGUE  SYYPNW
C
C
      IMPLICIT real (A-H,O-Z)
      INTEGER ICOPT,I,NEQ,ID
...
C All rights reserved.
C
      SUBROUTINE SDSTP(X,Y,YPRIME,NEQ,RES,JAC,PSOL,H,WT,VT,
...
C***DESCRIPTION
C
C     SDSTP solves a system of differential/algebraic equations of 
...
C                  in SDASPK prologue.
C     JAC --       External user-supplied routine to update
C                  Jacobian or preconditioner information in the
C                  nonlinear solver.  See JAC description in SDASPK
C                  prologue.
C     PSOL --      External user-supplied routine to solve
C                  a linear system using preconditioning. 
C                  (This is optional).  See PSOL in SDASPK prologue.
...
C                  See IDID description in SDASPK prologue.
C     RPAR,IPAR -- Real and integer parameter arrays that
C                  are used for communication between the
C                  calling program and external user routines.
C                  They are not altered by SNSK
C     PHI --       Array of divided differences used by
C                  SDSTP. The length is NEQ*(K+1), where
C                  K is the maximum order.
C     SAVR --      Work vector for SDSTP of length NEQ.
C     DELTA,E --   Work vectors for SDSTP of length NEQ.
C     WM,IWM --    Real and integer arrays storing
C                  information required by the linear solver.
C
C     The other parameters are information
C     which is needed internally by SDSTP to
C     continue from step to step.
C
C     RDAT,IDAT -- Are parameters to inquire about statistics while
C                  it is running (Added by JLM 3/7/92; updated 8/25/95)
C-----------------------------------------------------------------------
C***ROUTINES CALLED
C   NLS, SDWNRM, SDATRP
C
...
C
      IMPLICIT real(A-H,O-Z)
      INTEGER NEQ,LMXORD,LNST,LETF,LCFN,NCF,NEF,JSTART,K,KOLD,IPHASE,
     *  NS,KP1,KP2,KM1,NSP1,I,J,IDID,IERNLS,KNEW,IWM,KDIFF,J1,NONNEG,
     *  IDAT,NTYPE,JCALC,JFLG,IPAR
      DIMENSION Y(*),YPRIME(*),WT(*),VT(*)
      DIMENSION PHI(NEQ,*),SAVR(*),DELTA(*),E(*)
      DIMENSION WM(*),IWM(*)
      DIMENSION PSI(*),ALPHA(*),BETA(*),GAMMA(*),SIGMA(*)
      DIMENSION RPAR(*),IPAR(*), RDAT(*), IDAT(*)
...
C
C-----------------------------------------------------------------------
C  Declarations necessary for the inquiring while running.
      character*80 msgjm
...
C
      ENORM = SDWNRM(NEQ,E,VT,RPAR,IPAR)
      ERK = SIGMA(K+1)*ENORM
      TERK = (K+1)*ERK
      EST = ERK
      KNEW=K
...
405     DELTA(I) = PHI(I,KP1) + E(I)
      ERKM1=SIGMA(K)*SDWNRM(NEQ,DELTA,VT,RPAR,IPAR)
      TERKM1 = K*ERKM1
      IF(K .GT. 2)GO TO 410
...
415     DELTA(I) = PHI(I,K) + DELTA(I)
      ERKM2=SIGMA(K-1)*SDWNRM(NEQ,DELTA,VT,RPAR,IPAR)
      TERKM2 = (K-1)*ERKM2
...
430   CONTINUE
C----------------------------------------------------------------------
C     Special error message section JLM 3/7/92 ------------------------ 
ccc      ierrjm = ijmgetmr(msgjm,80,1,nrcv)
ccc      if (ierrjm .eq. 0) then
ccc         if (msgjm(1:nrcv).eq.'status' .or. msgjm(1:nrcv).eq.'s' ) then
c          big2 = 0.0e0
c          imxer2 = 1
c          do 452 i = 1,n
c             size2 = abs( acor(i)*ewt(i) )
c             if (big2 .ge. size2) go to 452
c             big2 = size2
c             imxer2 = i
c452       continue
c          big3 = 0.0e0
c          imxer3 = 1
c          do 453 i = 1,n
c             size2 = abs( savx(i)*ewt(i) )
c             if (big2 .ge. size2) go to 453
c             big3 = size2
c             imxer3 = i
c453       continue
...
C All rights reserved.
C
      SUBROUTINE SCNSTR (NEQ, Y, YNEW, ICNSTR, TAU, RLX, IRET, IVAR)
C
C***BEGIN PROLOGUE  SCNSTR
...
C-----------------------------------------------------------------------
      IMPLICIT real(A-H,O-Z)
      INTEGER NEQ,IRET,IVAR,I,ICNSTR
      DIMENSION Y(NEQ), YNEW(NEQ), ICNSTR(NEQ)
      SAVE FAC, FAC2, ZERO
      DATA FAC /0.6E0/, FAC2 /0.9E0/, ZERO/0.0E0/
C-----------------------------------------------------------------------
...
C All rights reserved.
C
      SUBROUTINE SCNST0 (NEQ, Y, ICNSTR, IRET)
C
C***BEGIN PROLOGUE  SCNST0
C***DATE WRITTEN   950808   (YYMMDD)
...
C All rights reserved.
C
      SUBROUTINE SINVWT(NEQ,WT,IER)
C
C***BEGIN PROLOGUE  SINVWT
C***REFER TO  SDASPK
C***ROUTINES CALLED  (NONE)
C***DATE WRITTEN   950125   (YYMMDD)
C***END PROLOGUE  SINVWT
C-----------------------------------------------------------------------
...
C
C-----------------------------------------------------------------------
C     The methods in subroutine SDSTP use polynomials
...
C     SDSTP, so SDATRP cannot be used alone.
C
...
C All rights reserved.
C
      real FUNCTION SDWNRM(NEQ,V,RWT,RPAR,IPAR)
C
C***BEGIN PROLOGUE  SDWNRM
C***ROUTINES CALLED  (NONE)
C***DATE WRITTEN   890101   (YYMMDD)
C***REVISION DATE  900926   (YYMMDD)
C***END PROLOGUE  SDWNRM
C-----------------------------------------------------------------------
...
C        SDWNRM=SQRT((1/NEQ)*SUM(V(I)*RWT(I))**2)
C-----------------------------------------------------------------------
C
      IMPLICIT real(A-H,O-Z)
      INTEGER I,NEQ,IPAR
...
20      SUM = SUM + ((V(I)*RWT(I))/VMAX)**2
      SDWNRM = VMAX*SQRT(SUM/NEQ)
30    CONTINUE
      RETURN
C
C------END OF FUNCTION SDWNRM-------------------------------------------
      END
C Work performed under the auspices of the U.S. Department of Energy
C by Lawrence Livermore National Laboratory under contract number 
C W-7405-Eng-48.
...
C All rights reserved.
C
      SUBROUTINE SDASID(X,Y,YPRIME,NEQ,ICOPT,ID,RES,JACD,PDUM,H,WT,
...
C
C
C     SDASID solves a nonlinear system of algebraic equations of the
...
C   RES, SMATD, SNSID
C
C***END PROLOGUE  SDASID
C
C
      IMPLICIT real(A-H,O-Z)
      INTEGER IPAR,LNRE,LNJE,LMXNIT,LMXNJ,MXNIT,IWM,MXNJ,IERNLS,NJ,
     * IRES,IERJ,IERNEW,JFDUM,ICNFLG,JSDUM,ICNSTR,NEQ,ID,ICOPT
...
C
      NJ = NJ + 1
      IWM(LNJE)=IWM(LNJE)+1
      CALL SMATD(NEQ,X,Y,YPRIME,DELTA,CJ,H,IERJ,WT,R,
...
C
      CALL SNSID(X,Y,YPRIME,NEQ,ICOPT,ID,RES,WT,RPAR,IPAR,DELTA,R,
...
C All rights reserved.
C
      SUBROUTINE SNSID(X,Y,YPRIME,NEQ,ICOPT,ID,RES,WT,RPAR,IPAR,
     *   DELTA,R,YIC,YPIC,WM,IWM,CJ,EPCON,RATEMX,MAXIT,STPTOL,
     *   ICNFLG,ICNSTR,IERNEW)
C
C***BEGIN PROLOGUE  SNSID
C***REFER TO  SDASPK
C***DATE WRITTEN   940701   (YYMMDD)
...
C***DESCRIPTION
C
C     SNSID solves a nonlinear system of algebraic equations of the
...
C                  length NEQ for SNSID.
C     WM,IWM    -- Real and integer arrays storing matrix information
C                  such as the matrix of partial derivatives,
C                  permutation vector, and various other information.
C     CJ        -- Matrix parameter = 1/H (ICOPT = 1) or 0 (ICOPT = 2).
C     R         -- Array of length NEQ used as workspace by the 
C                  linesearch routine SLINSD.
C     YIC,YPIC  -- Work vectors for SLINSD, each of length NEQ.
C     EPCON     -- Tolerance to test for convergence of the Newton
C                  iteration.
C     RATEMX    -- Maximum convergence rate for which Newton iteration
C                  is considered converging.
...
C
C***ROUTINES CALLED
C   SSLVD, SDWNRM, SLINSD, SCOPY
C
C***END PROLOGUE  SNSID
C
C
      IMPLICIT real(A-H,O-Z)
      INTEGER LNNI,LLSOFF,LSOFF,IWM,M,IRET,MAXIT,IERNEW,IRES,
     * ICNFLG,IPAR,ICNSTR,NEQ,ID,ICOPT
...
C
      CALL SSLVD (NEQ, DELTA, WM, IWM)
C
C     Get norm of DELTA.  Return now if norm(DELTA) .le. EPCON.
C
      DELNRM = SDWNRM(NEQ,DELTA,WT,RPAR,IPAR)
      FNRM = DELNRM
...
C
      OLDFNM = FNRM
C
      CALL SLINSD (NEQ, Y, X, YPRIME, CJ, DELTA, DELNRM, WT, LSOFF,
...
C All rights reserved.
C
      SUBROUTINE SLINSD (NEQ, Y, T, YPRIME, CJ, P, PNRM, WT, LSOFF,
     *                   STPTOL, IRET, RES, IRES, WM, IWM,
     *                   FNRM, ICOPT, ID, R, YNEW, YPNEW, ICNFLG,
     *                   ICNSTR, RLX, RPAR, IPAR)
C
C***BEGIN PROLOGUE  SLINSD
C***REFER TO  SNSID
C***DATE WRITTEN   941025   (YYMMDD)
C***REVISION DATE  941215   (YYMMDD)
C***REVISION DATE  960401   (Moved RL=ONE line to beginning.)
C
C
C-----------------------------------------------------------------------
C***DESCRIPTION
C
C     SLINSD uses a linesearch algorithm to calculate a new (Y,YPRIME)
...
C     ICNSTR  -- Integer array of length NEQ containing flags for
C                checking constraints.
C     RLX     -- Real scalar restricting update size in SCNSTR.
...
C
C***ROUTINES CALLED
C   SFNRMD, SYYPNW, SCOPY
C
C***END PROLOGUE  SLINSD
C
      IMPLICIT real(A-H,O-Z)
      INTEGER LNRE,LKPRIN,KPRIN,IWM,IVIO,ICNFLG,IRET,I,NEQ,LSOFF,
     * IRES,IPAR,ICNSTR,IVAR,ID,ICOPT
      EXTERNAL  RES
...
C
      SAVE ALPHA, ONE, TWO
      DATA ALPHA/1.0E-4/, ONE/1.0E0/, TWO/2.0E0/
C
      KPRIN=IWM(LKPRIN)
C
      F1NRM = (FNRM*FNRM)/TWO
      ...
      RL = ONE
      IF (KPRIN .GE. 2) THEN
        MSG = '------ IN ROUTINE SLINSD-- PNRM = (R1) )'
        CALL XERRWV(MSG, 40, 901, 0, 0, 0, 0, 1, PNRM, 0.0E0)
        ENDIF
      TAU = PNRM
      IVIO = 0
C-----------------------------------------------------------------------
C Check for violations of the contraints, if any are imposed.
...
C-----------------------------------------------------------------------
      IF (ICNFLG .NE. 0) THEN
 10      CONTINUE
         CALL SYYPNW (NEQ,Y,YPRIME,CJ,RL,P,ICOPT,ID,YNEW,YPNEW)
         CALL SCNSTR (NEQ, Y, YNEW, ICNSTR, TAU, RLX, IRET, IVAR)
         IF (IRET .EQ. 1) THEN
            IVIO = 1
            RATIO1 = TAU/PNRM
            RATIO = RATIO*RATIO1
 ...
 20           P(I) = P(I)*RATIO1
            ...
            IF (KPRIN .GE. 2) THEN
              ...
              CALL XERRWV(MSG, 50, 902, 0, 1, IVAR, 0, 1, PNRM, 0.0E0)
              ENDIF
...
C
      ...
      IF (LSOFF .EQ. 0 .AND. KPRIN .GE. 2) THEN
        MSG = '------ MIN. LAMBDA = (R1)'
        CALL XERRWV(MSG, 25, 903, 0, 0, 0, 0, 1, RLMIN, 0.0E0)
        ENDIF
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
 100  CONTINUE
      CALL SYYPNW (NEQ,Y,YPRIME,CJ,RL,P,ICOPT,ID,YNEW,YPNEW)
      CALL SFNRMD (NEQ, YNEW, T, YPNEW, R, CJ, WT, RES, IRES,
...
C
      ...
      IF (KPRIN .GE. 2) THEN
        MSG = '------ LAMBDA = (R1)'
        CALL XERRWV(MSG, 20, 904, 0, 0, 0, 0, 1, RL, 0.0E0)
        MSG = '------ NORM(F1) = (R1),  NORM(F1NEW) = (R2)'
        CALL XERRWV(MSG, 43, 905, 0, 0, 0, 0, 2, F1NRM, F1NRMP)
        ENDIF
...
C-----------------------------------------------------------------------
 150  IRET = 0
      CALL SCOPY (NEQ, YNEW, 1, Y, 1)
      CALL SCOPY (NEQ, YPNEW, 1, YPRIME, 1)
      FNRM = FNRMP
      IF (KPRIN .GE. 1) THEN
        MSG = '------ LEAVING ROUTINE SLINSD, FNRM = (R1)'
        CALL XERRWV(MSG, 42, 906, 0, 0, 0, 0, 1, FNRM, 0.0E0)
        ENDIF
      RETURN
C-----------------------------------------------------------------------
...
C All rights reserved.
C
      SUBROUTINE SFNRMD (NEQ, Y, T, YPRIME, R, CJ, WT, RES, IRES,
     *                   FNORM, WM, IWM, RPAR, IPAR)
C
C***BEGIN PROLOGUE  SFNRMD
C***REFER TO  SLINSD
C***DATE WRITTEN   941025   (YYMMDD)
C
C
...
C***DESCRIPTION
C
C     SFNRMD calculates the scaled preconditioned norm of the nonlinear
C     function used in the nonlinear iteration for obtaining consistent
C     initial conditions.  Specifically, SFNRMD calculates the weighted
...
C     SLINSD, the parameters represent
C
...
C   RES, SSLVD, SDWNRM
C
C***END PROLOGUE  SFNRMD
C
C
      IMPLICIT real (A-H,O-Z)
      INTEGER IRES,IWM,IPAR,NEQ
      EXTERNAL RES
      DIMENSION Y(*), YPRIME(*), WT(*), R(*)
      DIMENSION WM(*),IWM(*), RPAR(*),IPAR(*)
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      CALL SSLVD(NEQ,R,WM,IWM)
C-----------------------------------------------------------------------
C     Calculate norm of R.
C-----------------------------------------------------------------------
      FNORM = SDWNRM(NEQ,R,WT,RPAR,IPAR)
C
      RETURN
C----------------------- END OF SUBROUTINE SFNRMD ----------------------
      END
C Work performed under the auspices of the U.S. Department of Energy
C by Lawrence Livermore National Laboratory under contract number 
C W-7405-Eng-48.
...
C All rights reserved.
C
      SUBROUTINE SNEDD(X,Y,YPRIME,NEQ,RES,JACD,PDUM,H,WT,
...
C***DESCRIPTION
C
C     SNEDD solves a nonlinear system of
...
C     IDID      -- Completion flag, output by SNEDD.
C                  See IDID description in SDASPK prologue.
C     RPAR,IPAR -- Real and integer arrays used for communication
C                  between the calling program and external user
C                  routines.  They are not altered within DASPK.
C     PHI       -- Array of divided differences used by
C                  SNEDD.  The length is NEQ*(K+1),where
C                  K is the maximum order.
...
C     CJOLD     -- Saves the value of CJ as of the last call to SMATD.
C                  Accounts for changes in CJ needed to
C                  decide whether to call SMATD.
...
C                  preconditioner S = 100.E0, Thus the initial
...
C-----------------------------------------------------------------------
C***ROUTINES CALLED
C   SDWNRM, RES, SMATD, SNSD
C
C***END PROLOGUE  SNEDD
C
C
      IMPLICIT real(A-H,O-Z)
      
      INTEGER NEQ,LNRE,LNJE,IERTYP,NTYPE,JSTART,JCALC,IERNLS,IERJ,
     * IRES,IERNEW,I,J,KP1,IWM,MULDEL,MAXIT,NONNEG,IDID,JFDUM,IDUM,
     * IPAR
...
C
      ...
      DATA MULDEL/1/, MAXIT/4/, XRATE/0.25E0/
C
...
330   CONTINUE
      PNORM = SDWNRM (NEQ,Y,WT,RPAR,IPAR)
      TOLNEW = 100.E0*UROUND*PNORM
C     
C     Call RES to initialize DELTA.
C
      IWM(LNRE)=IWM(LNRE)+1
...
C
      IF(JCALC .EQ. -1) THEN
         IWM(LNJE)=IWM(LNJE)+1
         JCALC=0
         CALL SMATD(NEQ,X,Y,YPRIME,DELTA,CJ,H,IERJ,WT,E,WM,IWM,
...
C
      ...
      CALL SNSD(X,Y,YPRIME,NEQ,RES,PDUM,WT,RPAR,IPAR,DUMSVR,
...
C***DESCRIPTION
C
C     SNSD solves a nonlinear system of
C     algebraic equations of the form
C     G(X,Y,YPRIME) = 0 for the unknown Y.
C
C     The method used is a modified Newton scheme.
...
C                  routines.  They are not altered within DASPK.
C     DUMSVR    -- Dummy argument.
C     DELTA     -- Work vector for SNSD of length NEQ.
C     E         -- Error accumulation vector for SNSD of length NEQ.
C     WM,IWM    -- Real and integer arrays storing
C                  matrix information such as the matrix
C                  of partial derivatives, permutation
C                  vector, and various other information.
...
C                  in SDASPK prologue.  If IRES = -1, then IERNEW
C                  will be set to 1.
...
C
      IMPLICIT real(A-H,O-Z)
      INTEGER LNRE,LNNI,M,I,NEQ,IWM,MULDEL,MAXIT,IRES,IERNEW,IDUM,IPAR
...
C All rights reserved.
C
      SUBROUTINE SMATD(NEQ,X,Y,YPRIME,DELTA,CJ,H,IER,EWT,E,
     *                 WM,IWM,RES,IRES,UROUND,JACD,RPAR,IPAR)
C
C***BEGIN PROLOGUE  SMATD
C***REFER TO  SDASPK
C***DATE WRITTEN   890101   (YYMMDD)
C***REVISION DATE  900926   (YYMMDD)
...
C                are used for communication between the
C                calling program and external user routines.
C                They are not altered by SMATD.
C-----------------------------------------------------------------------
C***ROUTINES CALLED
C   JACD, RES, SGEFA, SGBFA
C
C***END PROLOGUE  SMATD
C
C
      IMPLICIT real(A-H,O-Z)
      INTEGER LML,LMU,LMTYPE,LNRE,LNPD,LLCIWP,LIPVT,IWM,IER,MTYPE,
     *  LENPD,I,IRES,NROW,NEQ,L,MEBAND,MBAND,MBA,MEB1,MSAVE,ISAVE,
     *  IPSAVE,J,N,K,I1,I2,II,IPAR
...
C
230      CALL SGEFA(WM,NEQ,NEQ,IWM(LIPVT),IER)
      RETURN
C
C
...
C
550   CALL SGBFA (WM,MEBAND,NEQ,IWM(LML),IWM(LMU),IWM(LIPVT),IER)
      RETURN
C
C------END OF SUBROUTINE SMATD------------------------------------------
      END
C Work performed under the auspices of the U.S. Department of Energy
C by Lawrence Livermore National Laboratory under contract number 
C W-7405-Eng-48.
...
C All rights reserved.
C
      SUBROUTINE SSLVD(NEQ,DELTA,WM,IWM)
C
C***BEGIN PROLOGUE  SSLVD
C***REFER TO  SDASPK
C***DATE WRITTEN   890101   (YYMMDD)
C***REVISION DATE  900926   (YYMMDD)
C***REVISION DATE  940701   (YYMMDD) (new LIPVT)
...
C     For a dense matrix, the LINPACK routine SGESL is called.
C     For a banded matrix, the LINPACK routine SGBSL is called.
C-----------------------------------------------------------------------
C***ROUTINES CALLED
C   SGESL, SGBSL
C
C***END PROLOGUE  SSLVD
C
c
      IMPLICIT real(A-H,O-Z)
      INTEGER LML,LMU,LMTYPE,LLCIWP,LIPVT,IWM,MTYPE,MEBAND,NEQ
...
C     Dense matrix.
C
100   CALL SGESL(WM,NEQ,NEQ,IWM(LIPVT),DELTA,0)
      RETURN
C
...
400   MEBAND=2*IWM(LML)+IWM(LMU)+1
      CALL SGBSL(WM,MEBAND,NEQ,IWM(LML),
     *  IWM(LMU),IWM(LIPVT),DELTA,0)
      RETURN
C
C------END OF SUBROUTINE SSLVD------------------------------------------
      END
...
C
C
C     SDASIK solves a nonlinear system of algebraic equations of the
C     form G(X,Y,YPRIME) = 0 for the unknown parts of Y and YPRIME in
C     the initial conditions.
C
...
C                  INFO(12) = 1 in the SDASPK prologue.
C     PSOL      -- External user-supplied routine to solve
C                  a linear system using preconditioning.
C                  (This is optional).  See explanation inside SDASPK.
C     H         -- Scaling factor for this initial condition calc.
C     WT        -- Vector of weights for error criterion.
...
C                  between the calling program and external user
C                  routines.  They are not altered within DASPK.
C     SAVR      -- Work vector for SDASIK of length NEQ.
C     DELTA     -- Work vector for SDASIK of length NEQ.
C     R         -- Work vector for SDASIK of length NEQ.
C     YIC,YPIC  -- Work vectors for SDASIK, each of length NEQ.
C     PWK       -- Work vector for SDASIK of length NEQ.
C     WM,IWM    -- Real and integer arrays storing
...
C                  See SDASPK prologue for more details.
...
C
C***ROUTINES CALLED
C   RES, JACK, SNSIK, SCOPY
C
C***END PROLOGUE  SDASIK
C
C
      IMPLICIT real(A-H,O-Z)
      INTEGER LNRE,LNJE,LLOCWP,LLCIWP,LMXNIT,LMXNJ,LWP,IWM,LIWP,
     *  MXNIT,MXNJ,IERNLS,NJ,IRES,IERPJ,IERNEW,JFLG,JSKIP,
     *  ICNFLG,IPAR,ICNSTR,NEQ,ID,ICOPT
      DIMENSION Y(*),YPRIME(*),ID(*),WT(*),ICNSTR(*)
...
C     Call the nonlinear Newton solver for up to MXNIT iterations.
C
      CALL SNSIK(X,Y,YPRIME,NEQ,ICOPT,ID,RES,PSOL,WT,RPAR,IPAR,
...
C***DESCRIPTION
C
C     SNSIK solves a nonlinear system of algebraic equations of the
C     form G(X,Y,YPRIME) = 0 for the unknown parts of Y and YPRIME in
C     the initial conditions.
C
...
C     PSOL      -- External user-supplied routine to solve
C                  a linear system using preconditioning. 
C                  See explanation inside SDASPK.
C     WT        -- Vector of weights for error criterion.
C     RPAR,IPAR -- Real and integer arrays used for communication
C                  between the calling program and external user
C                  routines.  They are not altered within DASPK.
C     SAVR      -- Work vector for SNSIK of length NEQ.
C     DELTA     -- Residual vector on entry, and work vector of
C                  length NEQ for SNSIK.
C     R         -- Work vector for SNSIK of length NEQ.
C     YIC,YPIC  -- Work vectors for SNSIK, each of length NEQ.
C     PWK       -- Work vector for SNSIK of length NEQ.
C     WM,IWM    -- Real and integer arrays storing
C                  matrix information such as the matrix
C                  of partial derivatives, permutation
C                  vector, and various other information.
...
C
C***ROUTINES CALLED
C   SFNRMK, SSLVK, SDWNRM, SLINSK, SCOPY
C
C***END PROLOGUE  SNSIK
C
C
      IMPLICIT real(A-H,O-Z)
      INTEGER LNNI,LNPS,LLOCWP,LLCIWP,LLSOFF,LSTOL,LSOFF,IWM,M,
     *  LWP,LIWP,IER,IERNEW,IRES,IERSL,IRET,MAXIT,ICNSTR,ID,
     *  ICNFLG,ICOPT,IPAR,NEQ
...
C
      CALL SCOPY (NEQ, DELTA, 1, SAVR, 1)
C
C     Compute norm of (P-inverse)*(residual).
C
      CALL SFNRMK (NEQ, Y, X, YPRIME, SAVR, R, CJ, WT, SQRTN, RSQRTN,
...
C
      CALL SSLVK (NEQ, Y, X, YPRIME, SAVR, DELTA, WT, WM, IWM,
...
C
      DELNRM = SDWNRM(NEQ,DELTA,WT,RPAR,IPAR)
      IF (DELNRM .EQ. 0.0E0) RETURN
C
C     Call linesearch routine for global strategy and set RATE.
C
      OLDFNM = FNRM
C
      CALL SLINSK (NEQ, Y, X, YPRIME, SAVR, CJ, DELTA, DELNRM, WT,
...
390   IF (IRES .LE. -2 .OR. IERSL .LT. 0) THEN
     ...
     1       .AND. RATE .LT. 1.0E0) IERNEW = 1
      ENDIF
      RETURN
C
C
C----------------------- END OF SUBROUTINE SNSIK------------------------
      END
C Work performed under the auspices of the U.S. Department of Energy
C by Lawrence Livermore National Laboratory under contract number 
C W-7405-Eng-48.
...
C***DESCRIPTION
C
C     SLINSK uses a linesearch algorithm to calculate a new (Y,YPRIME)
C     pair (YNEW,YPNEW) such that 
C
C     f(YNEW,YPNEW) .le. (1 - 2*ALPHA*RL)*f(Y,YPRIME) + 
...
C
C***ROUTINES CALLED
C   SFNRMK, SYYPNW, SCOPY
C
C***END PROLOGUE  SLINSK
C
      IMPLICIT real(A-H,O-Z)
      INTEGER LNRE,LNPS,LKPRIN,KPRIN,IWM,IVIO,ICNFLG,IRET,I,
     * NEQ,LSOFF,IRES,IER,ICNSTR,IVAR,ID,ICOPT,IPAR,IWP
      EXTERNAL  RES, PSOL
...
C
C-----------------------------------------------------------------------
C  Declarations necessary to kaboom out while running 9/25/95
      character*80 msgjm
      integer nrcv, ierrjm, ijmgetmr, imxer2, imxer3
C------------------------------------------------------------------------ 
...
C
      IF (KPRIN .GE. 2) THEN
        MSG = '------ IN ROUTINE SLINSK-- PNRM = (R1) )'
        CALL XERRWV(MSG, 40, 921, 0, 0, 0, 0, 1, PNRM, 0.0E0)
        ENDIF
      TAU = PNRM
      IVIO = 0
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
 ...
 20           P(I) = P(I)*RATIO1
            ...
            IF (KPRIN .GE. 2) THEN
              MSG = '------ CONSTRAINT VIOL., PNRM = (R1), INDEX = (I1)'
              CALL XERRWV(MSG, 50, 922, 0, 1, IVAR, 0, 1, PNRM, 0.0E0)
              ENDIF
            IF (PNRM .LE. STPTOL) THEN
              IRET = 1
              RETURN
...
C
      ...
      IF (LSOFF .EQ. 0 .AND. KPRIN .GE. 2) THEN
        MSG = '------ MIN. LAMBDA = (R1)'
        CALL XERRWV(MSG, 25, 923, 0, 0, 0, 0, 1, RLMIN, 0.0E0)
        ENDIF
C-----------------------------------------------------------------------
C Begin iteration to find RL value satisfying alpha-condition.
...
C-----------------------------------------------------------------------
 100  CONTINUE
      CALL SYYPNW (NEQ,Y,YPRIME,CJ,RL,P,ICOPT,ID,YNEW,YPNEW)
      CALL SFNRMK (NEQ, YNEW, T, YPNEW, SAVR, R, CJ, WT, SQRTN, RSQRTN,
...
C
      ...
      IF (KPRIN .GE. 2) THEN
        MSG = '------ LAMBDA = (R1)'
        CALL XERRWV(MSG, 20, 924, 0, 0, 0, 0, 1, RL, 0.0E0)
        MSG = '------ NORM(F1) = (R1),  NORM(F1NEW) = (R2)'
        CALL XERRWV(MSG, 43, 925, 0, 0, 0, 0, 2, F1NRM, F1NRMP)
        ENDIF
      IF (F1NRMP .GT. F1NRM + ALPHA*SLPI*RL) GO TO 200
C-----------------------------------------------------------------------
C Alpha-condition is satisfied, or linesearch is turned off.
...
C-----------------------------------------------------------------------
 150  IRET = 0
      ...
      FNRM = FNRMP
      IF (KPRIN .GE. 1) THEN
        MSG = '------ LEAVING ROUTINE SLINSK, FNRM = (R1)'
        CALL XERRWV(MSG, 42, 926, 0, 0, 0, 0, 1, FNRM, 0.0E0)
        ENDIF
C--------------------------------------------------------------------------
C     Special error message section to kaboom out: 9/25/95
ccc      ierrjm = ijmgetmr(msgjm,80,1,nrcv)
ccc      if (ierrjm .eq. 0) then
...
C All rights reserved.
C
      SUBROUTINE SFNRMK (NEQ, Y, T, YPRIME, SAVR, R, CJ, WT,
     *                   SQRTN, RSQRTN, RES, IRES, PSOL, IRIN, IER,
     *                   FNORM, EPLIN, WP, IWP, PWK, RPAR, IPAR)
C
C***BEGIN PROLOGUE  SFNRMK
C***REFER TO  SLINSK
C***DATE WRITTEN   940830   (YYMMDD)
...
C***DESCRIPTION
C
C     SFNRMK calculates the scaled preconditioned norm of the nonlinear
C     function used in the nonlinear iteration for obtaining consistent
C     initial conditions.  Specifically, SFNRMK calculates the weighted
C     root-mean-square norm of the vector (P-inverse)*G(T,Y,YPRIME),
C     where P is the preconditioner matrix.
C
C     In addition to the parameters described in the calling program
C     SLINSK, the parameters represent
C
...
C
C***ROUTINES CALLED
C   RES, SCOPY, SSCAL, PSOL, SDWNRM
C
C***END PROLOGUE  SFNRMK
C
C
      IMPLICIT real (A-H,O-Z)
      INTEGER IRIN,IRES,IER,IWP,IPAR,NEQ
      EXTERNAL RES, PSOL
...
C-----------------------------------------------------------------------
      CALL SCOPY(NEQ, SAVR, 1, R, 1)
      CALL SSCAL (NEQ, RSQRTN, WT, 1)
      IER = 0
      CALL PSOL (NEQ, T, Y, YPRIME, SAVR, PWK, CJ, WT, WP, IWP,
     *           R, EPLIN, IER, RPAR, IPAR)
      CALL SSCAL (NEQ, SQRTN, WT, 1)
      IF (IER .NE. 0) RETURN
C-----------------------------------------------------------------------
C     Calculate norm of R.
C-----------------------------------------------------------------------
...
C All rights reserved.
C
      SUBROUTINE SNEDK(X,Y,YPRIME,NEQ,RES,JACK,PSOL,
...
C***DESCRIPTION
C
C     SNEDK solves a nonlinear system of
C     algebraic equations of the form
C     G(X,Y,YPRIME) = 0 for the unknown Y.
C
...
C                  If JSTART = 0, then this is the first call,
C                  otherwise it is not.
C     IDID      -- Completion flag, output by SNEDK.
C                  See IDID description in SDASPK prologue.
C     RPAR,IPAR -- Real and integer arrays used for communication
C                  between the calling program and external user
C                  routines.  They are not altered within DASPK.
C     PHI       -- Array of divided differences used by
C                  SNEDK.  The length is NEQ*(K+1), where
C                  K is the maximum order.
C     GAMMA     -- Array used to predict Y and YPRIME.  The length
C                  is K+1, where K is the maximum order.
C     SAVR      -- Work vector for SNEDK of length NEQ.
C     DELTA     -- Work vector for SNEDK of length NEQ.
C     E         -- Error accumulation vector for SNEDK of length NEQ.
C     WM,IWM    -- Real and integer arrays storing
C                  matrix information for linear system
C                  solvers, and various other information.
C     CJ        -- Parameter always proportional to 1/H.
...
C     NONNEG    -- Flag to determine nonnegativity constraints.
C     NTYPE     -- Identification code for the SNEDK routine.
...
C-----------------------------------------------------------------------
C***ROUTINES CALLED
C   RES, JACK, SDWNRM, SNSK
C
C***END PROLOGUE  SNEDK
C
C
      IMPLICIT real(A-H,O-Z)
      INTEGER NEQ,LNRE,LNJE,LLOCWP,LLCIWP,IERTYP,NTYPE,JSTART,JCALC,
     *  IERNLS,LWP,IWM,LIWP,JFLG,IERPJ,IRES,IERSL,IERNEW,I,J,KP1,
     *  MULDEL,MAXIT,NONNEG,IDID,IPAR
...
C
      SAVE MULDEL, MAXIT, XRATE
      DATA MULDEL/0/, MAXIT/4/, XRATE/0.25E0/
C
C     Verify that this is the correct subroutine.
C
      IERTYP = 0
...
C     Call the nonlinear Newton solver.
C
      CALL SNSK(X,Y,YPRIME,NEQ,RES,PSOL,WT,RPAR,IPAR,SAVR,
...
C***DESCRIPTION
C
C     SNSK solves a nonlinear system of
C     algebraic equations of the form
C     G(X,Y,YPRIME) = 0 for the unknown Y.
C
C     The method used is a modified Newton scheme.
...
C                  between the calling program and external user
C                  routines.  They are not altered within DASPK.
C     SAVR      -- Work vector for SNSK of length NEQ.
C     DELTA     -- Work vector for SNSK of length NEQ.
C     E         -- Error accumulation vector for SNSK of length NEQ.
C     WM,IWM    -- Real and integer arrays storing
C                  matrix information such as the matrix
C                  of partial derivatives, permutation
C                  vector, and various other information.
...
C                  iteration.
C     S         -- Used for error convergence tests.
C                  In the Newton iteration: S = RATE/(1.E0-RATE),
C                  where RATE is the estimated rate of convergence
C                  of the Newton iteration.
C
C                  The closer RATE is to 0., the faster the Newton
...
C                  See IERSL description in subroutine SSLVK.
...
C
      IMPLICIT real(A-H,O-Z)
      INTEGER LNNI,LNRE,M,I,NEQ,IWM,MULDEL,IRES,IERSL,MAXIT,
     * IERNEW,IPAR
...
C All rights reserved.
C
      SUBROUTINE SSLVK (NEQ, Y, TN, YPRIME, SAVR, X, EWT, WM, IWM,
     *   RES, IRES, PSOL, IERSL, CJ, EPLIN, SQRTN, RSQRTN, RHOK,
     *   RPAR, IPAR)
C
C***BEGIN PROLOGUE  SSLVK
C***REFER TO  SDASPK
C***DATE WRITTEN   890101   (YYMMDD)
C***REVISION DATE  900926   (YYMMDD)
...
C***DESCRIPTION
C
C SSLVK uses a restart algorithm and interfaces to SSPIGM for
C the solution of the linear system arising from a Newton iteration.
C
C In addition to variables described elsewhere,
C communication with SSLVK uses the following variables..
...
C                    (SSPIGM returned IFLAG > 0.)
C         IERSL = -1 means there was a nonrecoverable error in the
C                    iterative solver, and an error exit will occur.
C-----------------------------------------------------------------------
C***ROUTINES CALLED
C   SSCAL, SCOPY, SSPIGM
C
C***END PROLOGUE  SSLVK
C
      INTEGER NEQ, IWM, IRES, IERSL, IPAR
      INTEGER LNRE,LNCFL,LNLI,LNPS,LLOCWP,LLCIWP,LMITER,LMAXL,
     *  LKMP,LNRMAX,LIWP,LWP,I,NRES
      real Y, TN, YPRIME, SAVR, X, EWT, WM, CJ, EPLIN,
...
C-----------------------------------------------------------------------
      ...
      LZ = LDL + NEQ
      CALL SSCAL (NEQ, RSQRTN, EWT, 1)
      CALL SCOPY (NEQ, X, 1, WM(LR), 1)
      DO 110 I = 1,NEQ
 110     X(I) = 0.E0
C-----------------------------------------------------------------------
...
C Update NRSTS; conditionally copy DL to R; call the SSPIGM
...
C-----------------------------------------------------------------------
 ...
 115  CONTINUE
      NRSTS = NRSTS + 1
      IF (NRSTS .GT. 0) CALL SCOPY (NEQ, WM(LDL), 1, WM(LR),1)
      CALL SSPIGM (NEQ, TN, Y, YPRIME, SAVR, WM(LR), EWT, MAXL, MAXLP1,
...
C-----------------------------------------------------------------------
      ...
      IWM(LNCFL) = NCFL
      IWM(LNRE)  = NRE
      CALL SSCAL (NEQ, SQRTN, EWT, 1)
      RETURN
C
C------END OF SUBROUTINE SSLVK------------------------------------------
      END
C Work performed under the auspices of the U.S. Department of Energy
C by Lawrence Livermore National Laboratory under contract number 
C W-7405-Eng-48.
...
C                destroyed.
C                (R is the same as V(*,MAXL+1) in the call to SSPIGM.)
C
...
C                call to SSPIGM.  If NRSTS .GT. 0, then the residual
...
C                matrix HES.
C
C         NRE  = The number of calls to RES (i.e. SATV)
C
...
C-----------------------------------------------------------------------
C***ROUTINES CALLED
C   PSOL, SNRM2, SSCAL, SATV, SORTH, SHEQR, SCOPY, SHELS, SAXPY
C
C***END PROLOGUE  SSPIGM
C
      INTEGER NEQ,MAXL,MAXLP1,KMP,IRES,NRE,NPSL,LGMR,IWP,
     1   IFLAG,IRST,NRSTS,IPAR,MAXLM1
      real TN,Y,YPRIME,SAVR,R,WGHT,EPLIN,CJ,Z,V,HES,Q,WP,WK,
     1   DL,RHOK,RPAR
      DIMENSION Y(*), YPRIME(*), SAVR(*), R(*), WGHT(*), Z(*),
     1   V(NEQ,*), HES(MAXLP1,*), Q(*), WP(*), IWP(*), WK(*), DL(*),
     2   RPAR(*), IPAR(*)
      INTEGER I, IER, INFO, IP1, I2, J, K, LL, LLP1
      real RNRM,C,DLNRM,PROD,RHO,S,SNORMW,SNRM2,TEM
      EXTERNAL  RES, PSOL
C
      IER = 0
      IFLAG = 0
...
C Call routine SORTH to orthogonalize the new vector VNEW = V(*,LL+1).
C call routine SHEQR to update the factors of HES.
C-----------------------------------------------------------------------
        CALL SATV (NEQ, Y, TN, YPRIME, SAVR, V(1,LL), WGHT, Z,
     1     RES, IRES, PSOL, V(1,LL+1), WK, WP, IWP, CJ, EPLIN,
     1     IER, NRE, NPSL, RPAR, IPAR)
        IF (IRES .LT. 0) RETURN
        IF (IER .NE. 0) GO TO 300
        CALL SORTH (V(1,LL+1), V, HES, NEQ, LL, MAXLP1, KMP, SNORMW)
        HES(LL+1,LL) = SNORMW
...
C-----------------------------------------------------------------------
 ...
 210    R(K) = 0.0E0
      R(1) = RNRM
      CALL SHELS (HES, MAXLP1, LL, Q, R)
      DO 220 K = 1,NEQ
 220    Z(K) = 0.0E0
      DO 230 I = 1,LL
        CALL SAXPY (NEQ, R(I), V(1,I), 1, Z, 1)
 230    CONTINUE
      DO 240 I = 1,NEQ
 240    Z(I) = Z(I)/WGHT(I)
...
C All rights reserved.
C
      SUBROUTINE SATV (NEQ, Y, TN, YPRIME, SAVR, V, WGHT, YPTEM, RES,
     *   IRES, PSOL, Z, VTEM, WP, IWP, CJ, EPLIN, IER, NRE, NPSL,
     *   RPAR,IPAR)
C
C***BEGIN PROLOGUE  SATV
C***DATE WRITTEN   890101   (YYMMDD)
C***REVISION DATE  900926   (YYMMDD)
C
C
...
C
      ...
      real Y, TN, YPRIME, SAVR, V, WGHT, YPTEM, Z, VTEM,
     1   WP, CJ, RPAR
...
C-----------------------------------------------------------------------
      DO 20 I = 1,NEQ
        YPTEM(I) = YPRIME(I) + VTEM(I)*CJ*scaleps
 20     Z(I) = Y(I) + VTEM(I)*scaleps
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
 ...
 30     CONTINUE
      ...
      RETURN
C
C------END OF SUBROUTINE SORTH------------------------------------------
      END
C
C ... subroutines SHEQR and SHELS are now in file svrut3.m
C
      real FUNCTION R1MACH8 (IDUM)
      INTEGER IDUM
C-----------------------------------------------------------------------
